function [h, mu, sigma, exitFlag, fval] = gfit(param0)
...
options = optimset('MaxFunEvals', 400 * numel(param0), ...
                   'Display', 'off');
[pfit, fval, exitFlag] = fminsearch(@costFun, param0, options);
[h, mu, sigma] = deal(pfit(1), pfit(2), pfit(3));
end

% Define the Gaussian function
function y = gpdf(h, mu, sigma)
    global pixIndex
    y = h .* exp(-(pixIndex - mu) .^ 2 / (2 * sigma ^ 2));
end

function cost = costFun(p)
    global pixHist
    %h = p(1)*p(4)*exp(1);%/ncdf(( - (p(3) ^ 2 / p(4))) ./ p(3))
    cost = sum(abs(gpdf(p(1), p(2), p(3)) - pixHist));
end